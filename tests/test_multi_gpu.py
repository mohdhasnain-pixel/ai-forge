...
class TestResolveNumGpus:
    ...
    def test_resolve_integer(self):
        from ai_forge_cli.utils.topology import resolve_num_gpus
    ...
    def test_resolve_auto_uses_detected(self):
        ...
        with patch(
            "ai_forge_cli.utils.topology._detected_gpu_count", return_value=8
        ):
    ...
    def test_resolve_auto_cpu_returns_zero(self):
        ...
        with patch(
            "ai_forge_cli.utils.topology._detected_gpu_count", return_value=0
        ):
...
class TestDetectTopology:
    ...
    def test_no_cuda_returns_none_interconnect(self):
        from ai_forge_cli.utils.topology import detect_topology
    ...
    def test_single_gpu_has_no_interconnect(self):
        ...
        with patch(
            "ai_forge_cli.utils.topology._detected_gpu_count", return_value=1
        ):
            topo = detect_topology()
    ...
    def test_multi_gpu_nvlink_detected(self):
        ...
        with (
            patch(
                "ai_forge_cli.utils.topology._detected_gpu_count", return_value=4
            ),
            patch(
                "ai_forge_cli.utils.topology._count_nvlink_pairs", return_value=6
            ),
        ):
            topo = detect_topology()
    ...
    def test_multi_gpu_pcie_fallback(self):
        ...
        with (
            patch(
                "ai_forge_cli.utils.topology._detected_gpu_count", return_value=2
            ),
            patch(
                "ai_forge_cli.utils.topology._count_nvlink_pairs", return_value=0
            ),
        ):
            topo = detect_topology()
...
class TestSuggestStrategy:
    ...
    def test_single_gpu_no_strategy(self):
        from ai_forge_cli.utils.topology import suggest_strategy
...
class TestLauncherDetectActive:
    ...
    def test_not_in_distributed(self):
        from ai_forge_cli.utils.launcher import is_in_distributed
...
class TestLauncherAdvice:
    ...
    def test_format_advice_command(self):
        from ai_forge_cli.utils.launcher import format_advice
...
class TestFsdp2Compile:
    ...
    def test_validate_compile_ok(self):
        ...
        with patch("ai_forge_cli.utils.fsdp.is_fsdp_available", return_value=True):
            errors = validate_fsdp2_compile_config(
                use_compile=True,
...
class TestMiiBackend:
    ...
    def test_is_mii_available_false_when_stubbed_to_none(self):
        from ai_forge_cli.utils.mii import is_mii_available
    ...
    def test_create_mii_pipeline_mocked(self):
        from ai_forge_cli.utils.mii import create_mii_pipeline
...
class TestFsdp2CompileHelper:
    ...
    def test_no_fsdp_leaves_kwargs_untouched(self):
        from ai_forge_cli.utils.fsdp import apply_fsdp_training_kwargs
