...
1. builds an image with ``ai-forge[train]`` pinned to the running version,
...
# break out of the embedded ``pip_install("ai-forge[train]==<ver>")`` string.
...
def render_modal_stub(
    ...
    *,
    ...
    pip_spec = f"ai-forge[train]=={soup_version}"
...
def submit_modal_run(plan: CloudPlan, *, env: Optional[Mapping] = None) -> int:
    ...
    try:
        ...
        raise RuntimeError(
            "Modal SDK not installed. Run `pip install \"ai-forge[modal]\"`."
        ) from exc
    import subprocess
