...
Bridges :mod:`ai_forge_cli.plugins` registered hooks into the HF Trainer
...
def _collect_active_hooks() -> list[tuple[str, dict[str, Any]]]:
    ...
    Lazy-imports :mod:`ai_forge_cli.plugins` so the callback module stays
    cheap to import in CI / non-training contexts.
    """
    from ai_forge_cli.plugins import discover_hooks, list_plugins
