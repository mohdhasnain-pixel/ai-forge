...
'ai-forge[compile]'``) so the command works without them installed.
...
The CLI command lives in ``ai_forge_cli/commands/compile_cmd.py`` (named
...
_INSTALL_HINT = (
    "Run: pip install \"ai-forge[compile]\"  (installs dspy-ai / textgrad / gepa)"
)
...
def run_compile(plan: CompilePlan) -> CompileResult:
    ...
    friendly ``ImportError`` (naming ``pip install ai-forge[compile]``) when it
