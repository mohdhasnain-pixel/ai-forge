...
Wraps v0.26.0 :mod:`ai_forge_cli.eval.quant_check` with disk-cache so that
...
def compute_cache_key(
    *,
    ...
    digest inputs. The orchestrator in :mod:`ai_forge_cli.commands.deploy`
...
def _score_tasks(
    ...
    from ai_forge_cli.eval.custom import load_eval_tasks, score_task
...
def _load_measure_model(
    ...
    *,
    ...
    :func:`ai_forge_cli.utils.live_eval.make_generator`'s ``loaded=`` kwarg.
    ``quantization='none'`` loads the plain baseline. Heavy imports are lazy.
    """
    from ai_forge_cli.config.schema import TrainingConfig
    from ai_forge_cli.utils.live_eval import resolve_device
...
def build_before_generator(
    ...
    *,
    ...
    :func:`ai_forge_cli.utils.live_eval.make_generator`) on the first prompt —
