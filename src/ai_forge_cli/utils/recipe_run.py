...
- ``llm_text`` + ``judge`` route through the v0.20.0 provider catalog via
  :func:`ai_forge_cli.utils.data_forge.make_judge_provider_fn` (so the same
  SSRF / env-key surface applies).
- ``code`` reuses the v0.25.0 RLVR sandbox via
  :func:`ai_forge_cli.trainer.rewards._run_code_sandbox`.
"""
...
_LOG = logging.getLogger("ai_forge_cli.utils.recipe_run")
...
def run_recipe(
    ...
    *,
    ...
    from ai_forge_cli.utils.recipe_dag import RecipeDAG
