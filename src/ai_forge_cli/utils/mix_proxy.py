...
recipe, invokes ``python -m ai_forge_cli.cli train --config <tmp> --yes`` via a
...
- Per-candidate temp dir lives under ``tempfile.gettempdir()``; cleaned up
  ...
  policy).
- Subprocess invocation uses ``sys.executable + ['-m', 'ai_forge_cli.cli', ...]``
...
def _validate_path_under_cwd(name: str, raw: object) -> str:
    ...
    from ai_forge_cli.utils.paths import is_under_cwd  # noqa: PLC0415
...
def _read_final_eval_loss(
    ...
    """
    from ai_forge_cli.experiment.tracker import ExperimentTracker  # noqa: PLC0415
