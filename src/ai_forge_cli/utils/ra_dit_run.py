...
def discover_latest_retriever(*, store: Any = None) -> Optional[str]:
    ...
    """
    from ai_forge_cli.utils.ra_dit import (  # noqa: PLC0415
    ...
    if store is None:
        try:
            from ai_forge_cli.registry.store import RegistryStore  # noqa: PLC0415
...
def validate_ra_dit_config_path(name: str, raw: str) -> str:
    ...
    """
    from ai_forge_cli.utils.paths import enforce_under_cwd_and_no_symlink  # noqa: PLC0415
...
def _run_train_subprocess(config_path: str, *, timeout_seconds: int) -> None:
    """Default stage runner: ``python -m ai_forge_cli.cli train --config <p> --yes``.
