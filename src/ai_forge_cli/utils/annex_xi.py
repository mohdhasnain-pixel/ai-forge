...
_Generated by ai-forge {_md_escape(data.soup_version)} at {_md_escape(data.created_at)}._
...
def render_annex_pdf(data: AnnexXIData, section: str) -> bytes:
    ...
    Lazy-imports ``reportlab`` (install via ``pip install ai-forge[pdf]``).
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "`pip install ai-forge[pdf]` (or `pip install reportlab`)."
        ) from exc
