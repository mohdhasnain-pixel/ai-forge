...
from ai_forge_cli.utils.paths import atomic_write_text, is_under_cwd
...
_PACKAGING_MISSING_WARNING = (
    ...
    "declared bounds. Reinstall ai-forge to restore it."
)
...
# (a training venv later contaminated by `pip install "ai-forge[serve-fast]"`).
# `[all]` and `[dev]` both re-declare `ai-forge[train,...]`, which pip flattens,
...
def check_declared_bounds(
    ...
    for raw in requirements:
        ...
        # opted into `ai-forge[X]`; evaluating in the base environment (no extra
...
def snapshot_env() -> EnvLock:
    ...
    return EnvLock(
        soup_version=ai_forge_cli.__version__,
        python_version=py_v,
        platform=plat,
        cuda_version=cuda_v,
...
def compute_env_hash(lock: EnvLock) -> str:
    ...
    :func:`ai_forge_cli.utils.soup_lock.compute_lock_closure`.
    """
    if not isinstance(lock, EnvLock):
        raise TypeError(f"lock must be EnvLock, got {type(lock).__name__}")
    payload = {
