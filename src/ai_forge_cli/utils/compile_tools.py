...
``ImportError`` (``pip install "ai-forge[compile]"``).
...
_INSTALL_HINT = (
    "Run: pip install \"ai-forge[compile]\"  (installs textgrad / gepa)"
)
...
def run_tool_compile(plan: ToolCompilePlan) -> int:
    ...
    from ai_forge_cli.utils.agent_forge import load_spec_file, parse_spec
