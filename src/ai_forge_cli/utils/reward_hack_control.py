...
:mod:`ai_forge_cli.utils.reward_hacking` detector) and SELF-CORRECTS instead of
only halting:
...
- ``RewardHackMitigationCallback`` lazily resolves ``transformers.TrainerCallback``
  (falls back to ``object``), mirroring :mod:`ai_forge_cli.utils.reward_hacking`.

Security:
- ``MitigationLogWriter`` mirrors :class:`ai_forge_cli.monitoring.trace_logger.TraceLogWriter`:
...
from ai_forge_cli.monitoring.trace_logger import redact_value
...
def _completion_text(completion: Any) -> str:
    ...
    from ai_forge_cli.utils.rl_signal_buffer import _completion_to_text
...
def mean_repetition(completions: Sequence[Any]) -> float:
    ...
    Reuses :func:`ai_forge_cli.utils.echo_trap.score_echo_signal` (lazy import —
...
class MitigationLogWriter:
    ...
    Mirrors :class:`ai_forge_cli.monitoring.trace_logger.TraceLogWriter`.
    """
...
class RewardHackMitigationCallback(_TrainerCallbackBase):  # type: ignore[misc, valid-type]
    ...
    Reads the shared :class:`~ai_forge_cli.utils.rl_signal_buffer.RLSignalBuffer`
    snapshot each step, computes a multi-signal hacking score (reusing the
    v0.70.0 :class:`~ai_forge_cli.utils.reward_hacking.RewardHackCallback` for the
