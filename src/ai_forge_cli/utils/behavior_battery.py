...
``ai_forge_cli/data/_fixtures/behavior/<name>.jsonl`` so the surface is usable
...
def load_battery_probes(name: str) -> tuple[dict, ...]:
    ...
    ``ai_forge_cli/data/_fixtures/behavior/<name>.jsonl``.

    Uses ``importlib.resources.files("ai_forge_cli")`` ``Traversable`` ``/``
