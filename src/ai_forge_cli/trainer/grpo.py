...
def make_grpo_trainer_variant(base_cls: type, variant: str) -> type:
    ...
    through :func:`ai_forge_cli.utils.grpo_variants.apply_variant_loss`. Cached
    ...
    MEDIUM fix).
    """
    from ai_forge_cli.utils.grpo_variants import validate_grpo_variant
...
def _select_reward_fn(
    ...
    if tcfg.prm_reward is not None:
        from ai_forge_cli.utils.prm_reward import build_prm_reward_fn
...
class GRPOTrainerWrapper:
    ...
    def setup(self, dataset: dict):
        ...
        if rl_callbacks_need_buffer(tcfg):
            ...
            from ai_forge_cli.utils.rl_signal_buffer import (
                RLSignalBuffer,
                wrap_reward_funcs,
            )
