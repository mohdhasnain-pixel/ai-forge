...
from ai_forge_cli.utils.gpu import (
    bf16_fp16_flags,
    estimate_batch_size,
    model_size_from_name,
    resolve_device_map,
)
from ai_forge_cli.utils.seeding import apply_training_seed, training_seed_kwargs
...
class SFTTrainerWrapper(StreamingSetupMixin):
    ...
    def setup(self, dataset: dict):
        ...
        if batch_size == "auto":
            ...
            from ai_forge_cli.utils.gpu import get_gpu_info
        ...
        if self._is_raft:
            ...
            from ai_forge_cli.trainer.raft import (
                RaftDataCollator,
    ...
    def _prepare_raft_dataset(self, dataset: dict, cfg, tcfg):
        ...
        (:func:`ai_forge_cli.utils.raft.build_raft_prompt`) then tokenised with the
        prompt span masked. When ``citation_faithful`` is set, citation spans
        in the answer get a boosted ``loss_weights`` entry.
        """
        from datasets import Dataset

        from ai_forge_cli.utils.raft import build_raft_prompt, tokenize_raft_example
    ...
    def _prepare_raft_raw_dataset(self, dataset: dict, cfg, tcfg):
        ...
        :class:`~ai_forge_cli.trainer.raft.RaftEpochShuffleCollator` re-composes +
    ...
    def _setup_transformers(self, cfg, tcfg):
        ...
        from ai_forge_cli.utils.moe import detect_moe_model, get_moe_target_modules
        ...
        if tcfg.use_cut_ce:
            from ai_forge_cli.utils.cut_ce import NO_MATCHING_ARCHITECTURE_MESSAGE, apply_cut_ce
        ...
        # Quantization (v0.38.0 Quant Menu — see ai_forge_cli.utils.quant_menu)
        ...
        from ai_forge_cli.utils.data_pipeline import apply_vocab_expansion
    ...
    def _setup_unsloth(self, cfg, tcfg):
        ...
        from ai_forge_cli.utils.unsloth import load_model_and_tokenizer
    ...
    def _prepare_audio_dataset(self, dataset: dict):
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install with: pip install \"ai-forge[audio]\""
            )
