...
- **trl 0.19.x** — ``from trl import OnlineDPOTrainer``; the judge is a
  ...
  :func:`ai_forge_cli.eval.judge.make_soup_pairwise_judge`); a reward model is
...
- **trl 1.x** — pairwise judges were removed; ``OnlineDPOTrainer`` moved to
  ...
  (:func:`ai_forge_cli.eval.judge.make_judge_reward_func`) — the same pointwise judge
...
def _trl_accepts(param: str) -> bool:
    ...
    ``pip install "ai-forge[train]"`` resolves — and the wrapper passed a keyword
...
class OnlineDPOTrainerWrapper:
    ...
    def _build_judge_or_reward(self, tcfg) -> dict:
        ...
        if tcfg.online_dpo_judge:
            ...
            from ai_forge_cli.eval.judge import JudgeEvaluator, validate_judge_api_base
