...
from ai_forge_cli.cans.unpack import extract_can, inspect_can
...
def _deploy_target(target: DeployTarget, extract_dir: Path) -> int:
    ...
    if target.kind == "ollama":
        ...
        return _run_subprocess([
            sys.executable, "-m", "ai_forge_cli.cli",
            "deploy", "ollama",
            "--gguf", str(gguf_path),
            "--name", target.name,
        ])
...
def run_can(
    ...
    train_argv = [
        sys.executable, "-m", "ai_forge_cli.cli", "train",
