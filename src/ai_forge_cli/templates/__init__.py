...
``ai_forge_cli.config.schema`` for back-compat.
"""
...
def list_templates() -> list[str]:
    ...
    try:
        from ai_forge_cli.config.schema import TEMPLATES as _INLINE
...
def load_template(name: str) -> Optional[str]:
    ...
    Order:
      ...
      2. Inline ``TEMPLATES`` dict in ``ai_forge_cli.config.schema``.
    """
    _validate_name(name)
    manifest = _load_manifest()
