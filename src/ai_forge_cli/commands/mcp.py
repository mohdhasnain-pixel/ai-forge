...
def serve(
    ...
    try:
        from ai_forge_cli.mcp_server.server import run_stdio_server
    except ImportError:
        # NB: escape the '[' in 'ai-forge[mcp]' so Rich prints it literally
        # instead of parsing '[mcp]' as a (dropped) markup tag.
        console.print(
            "[red]The MCP server needs the 'mcp' SDK.[/] "
            "Install it with: [bold]pip install \"ai-forge\\[mcp]\"[/]"
        )
