...
from ai_forge_cli.utils.gpu import detect_device, get_gpu_info, resolve_quantization
...
def _hardware_fit_preflight(cfg, gpu_info, *, allow_oom_attempt: bool) -> None:
    ...
    if inp is None:
        return
    from ai_forge_cli.utils.hardware_fit import VRAM_SAFETY_MARGIN, decide_hardware_fit
...
def train(
    ...
    track_energy: bool = typer.Option(
        ...
        help=(
            ...
            "(offline; requires `pip install ai-forge\\[carbon]`). Feeds the "
    ...
    if find_lr:
        from ai_forge_cli.utils.lr_finder import (
            compute_lr_schedule,
    ...
    if cloud:
        from ai_forge_cli import __version__ as _soup_version
        from ai_forge_cli.cloud import modal as _modal_cloud
    ...
    if hf_resume and push_as and resume_from is None:
        ...
        from ai_forge_cli.utils.hf import resolve_endpoint, resolve_token
    ...
    if wandb:
        try:
            ...
            console.print(
                ...
                "Run: [bold]pip install \"ai-forge\\[wandb]\"[/]"
            )
            raise typer.Exit(1)
    ...
    fsdp_kwargs = None
    if fsdp:
        from ai_forge_cli.utils.fsdp import FSDP_CONFIGS, get_fsdp_training_args
    ...
    num_gpus = None
    if gpus:
        from ai_forge_cli.utils.topology import detect_topology, resolve_num_gpus
        ...
        if num_gpus is not None and num_gpus < 1:
            ...
            if dry_run and not is_in_distributed():
                ...
                from ai_forge_cli.utils.topology import suggest_nccl_env
    ...
    if cfg.training.use_fsdp2_compile:
        from ai_forge_cli.utils.fsdp import validate_fsdp2_compile_config
    ...
    if cfg.training.parallelism == "pipeline":
        from ai_forge_cli.utils.pipeline import validate_pipeline_config
    ...
    from ai_forge_cli.utils.trackers import resolve_report_to
    ...
    if profile_run:
        from ai_forge_cli.utils.profiling import profile_training
    ...
    try:
        ...
        try:
            from ai_forge_cli.utils.crash import build_crash_bundle, write_crash_bundle
    ...
    if track_energy:
        if energy_measurement is not None:
            ...
            console.print(
                ...
                "(install `pip install ai-forge\\[carbon]`)"
            )
    ...
    if energy_out and _should_run_diagnose_gate_on_rank():
        if energy_measurement is not None:
            ...
            console.print(
                ...
                "(set --track-energy + install `pip install ai-forge\\[carbon]`)"
            )
...
def _run_diagnose_gate(
    ...
    from ai_forge_cli.utils.diagnose.report import FAILURE_MODES, FailureScore
    ...
    from ai_forge_cli.utils.diagnose.report import classify_score
...
def _resolve_deepspeed(deepspeed: str) -> str:
    ...
    from ai_forge_cli.utils.deepspeed import CONFIGS, write_deepspeed_config
