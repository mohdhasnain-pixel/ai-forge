...
from ai_forge_cli.commands.push import (
...
def build_model_card(
    ...
    fm.append("tags:")
    for tag in ("ai-forge", "fine-tuned", "compliance"):
        fm.append(f"  - {tag}")
    fm.append("---")

    lines: list[str] = list(fm)
    lines += ["", f"# {_safe_md_cell(name)}", ""]
    lines.append("Fine-tuned with [Soup CLI](https://github.com/mohdhasnain-pixel/ai-forge).")
    lines.append("")
