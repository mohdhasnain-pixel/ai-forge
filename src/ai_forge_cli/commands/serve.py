...
def _validate_adapter_path(path: str, cwd: Optional[str] = None) -> bool:
    ...
    from ai_forge_cli.utils.paths import is_under
...
def serve(
    ...
    try:
        ...
        console.print(
            ...
            "Install with: [bold]pip install \"ai-forge\\[serve]\"[/]"
        )
        raise typer.Exit(1)
    ...
    if backend == "mii":
        from ai_forge_cli.utils.mii import (
            build_mii_app,
            create_mii_pipeline,
            is_mii_available,
        )
    ...
    if backend == "vllm":
        ...
        if not is_vllm_available():
            console.print(
                ...
                "Install with: [bold]pip install \"ai-forge\\[serve-fast]\"[/]"
            )
            raise typer.Exit(1)
    ...
    if backend == "sglang":
        ...
        if not check_sglang_available():
            console.print(
                ...
                "Install with: [bold]pip install \"ai-forge\\[sglang]\"[/]"
            )
            raise typer.Exit(1)
    ...
    if auto_quant:
        from ai_forge_cli.utils.auto_quant import (
    ...
    if backend == "vllm":
        ...
        if mole is not None:
            ...
            from ai_forge_cli.utils.mole_routing import load_mole_for_serve
        ...
        if trace_log is not None:
            from ai_forge_cli.monitoring.trace_logger import TraceLogWriter
        ...
        if reasoning_parser:
            from ai_forge_cli.utils.reasoning_parser import validate_parser_name
        ...
        if record_thumbs is not None:
            ...
            from ai_forge_cli.utils.local_rl import init_local_rl_db, validate_db_path
...
def _serve_vllm(
    ...
    from ai_forge_cli.utils.vllm import create_vllm_app, create_vllm_engine
...
def _serve_sglang(
    ...
    from ai_forge_cli.utils.sglang import create_sglang_app, create_sglang_runtime
...
def _create_app(
        ...
        with _contextlib.ExitStack() as stack:
            ...
            try:
                try:
                    ...
                    from ai_forge_cli.utils.structured_output import (
    ...
    def tool_python(
        ...
        if len(code) > tool_max_code_len:
            raise HTTPException(status_code=400, detail="Invalid request")
        try:
            from ai_forge_cli.trainer.rewards import _run_code_sandbox
    ...
    def record_thumb_endpoint(
        ...
        if (
            ...
            raise HTTPException(status_code=400, detail="Invalid request")
        from ai_forge_cli.utils.local_rl import record_thumb
