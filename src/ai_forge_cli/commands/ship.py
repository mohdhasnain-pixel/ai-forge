...
if TYPE_CHECKING:  # pydantic models — import for typing only (no eager cost)
    from ai_forge_cli.config.schema import ShipConfig, SoupConfig
...
def _leg1_metric(
    ...
    from ai_forge_cli.eval.custom import load_eval_tasks, run_eval
...
def _build_pairwise_scorer(
    ...
    from ai_forge_cli.eval.custom import load_eval_tasks
    from ai_forge_cli.eval.gate import _parse_judge_url
    from ai_forge_cli.eval.judge import JudgeEvaluator, pairwise_winrate
...
def _measure_noise_floor(
    ...
    *,
    ...
    """
    from ai_forge_cli.eval.gate_suites import is_bundled_suite, score_bundled_suite
