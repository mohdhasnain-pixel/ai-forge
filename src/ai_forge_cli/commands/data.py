...
from ai_forge_cli.utils.embed import DEFAULT_EMBED_MODEL, embed_texts
from ai_forge_cli.utils.paths import is_under_cwd
from ai_forge_cli.utils.semdedup import DedupReport, greedy_semdedup
    ...
    *,
    ...
    try:
        ...
        console.print(
            ...
            # markup tag and print `pip install "ai-forge"` -- a command that
            ...
            "Install with: [bold]pip install \"ai-forge\\[train]\"[/]"
        )
        raise typer.Exit(1)
...
def dedup(
    ...
    semantic: bool = typer.Option(
        ...
        help="Use embedding cosine (SemDeDup) instead of MinHash. Catches "
             "paraphrases MinHash misses. Requires ai-forge\\[train].",
    ),
    ...
    try:
        ...
        console.print(
            ...
            "Install with: [bold]pip install \"ai-forge\\[data]\"[/]"
        )
        raise typer.Exit(1)
...
def filter_data(
    ...
    if perplexity is not None or score_only:
        try:
            from ai_forge_cli.utils.quality import compute_perplexity_scores
...
def download_dataset(
    ...
    if hub_canonical != "hf":
        from ai_forge_cli.utils.hubs import download_repo as _download_repo
        ...
        try:
            if hub_canonical == "modelscope":
                ...
                console.print(
                    ...
                    "use ai_forge_cli.utils.hubs.download_repo for raw "
                    "snapshot download.[/]"
                )
            else:  # modelers
                try:
...
def _get_registry_path() -> Path:
    ...
    from ai_forge_cli.utils.registry import _default_registry_path
...
def augment_data(
    ...
    from ai_forge_cli.data.augment import STRATEGIES
...
def _load_augment_provider(
    ...
    *,
    ...
    from ai_forge_cli.utils.data_forge import JUDGE_PROVIDERS, make_judge_provider_fn
...
def register_data(
    ...
    """
    from ai_forge_cli.utils.registry import register_dataset
...
def unregister_data(
    ...
    """
    from ai_forge_cli.utils.registry import unregister_dataset
...
def from_traces_cmd(
    ...
    from ai_forge_cli.data.traces import (
        SUPPORTED_FORMATS,
        SUPPORTED_SIGNALS,
        build_pairs,
        parse_langchain,
    ...
    from ai_forge_cli.utils.paths import is_under_cwd as _under_cwd
    ...
    if judge:
        ...
        from ai_forge_cli.data.traces.quality import judge_filter_pairs
        from ai_forge_cli.eval.judge import VALID_PROVIDERS, JudgeEvaluator
...
def list_registry():
    ...
    from ai_forge_cli.utils.registry import load_registry
...
def push_dataset_cmd(
    ...
    from ai_forge_cli.utils.hf import (
        get_hf_api,
        resolve_endpoint,
        resolve_token,
        validate_repo_id,
...
def ingest_document(
    ...
    from ai_forge_cli.utils.data_pipeline import detect_ingest_format
    from ai_forge_cli.utils.paths import is_under_cwd
...
def recipe(
    ...
    from ai_forge_cli.utils.recipe_dag import load_recipe_yaml
...
def gen_magpie(
    ...
    from ai_forge_cli.utils.magpie import build_magpie_config, run_magpie
...
def best_of_n(
    ...
    from ai_forge_cli.utils import best_of_n as bon
    from ai_forge_cli.utils.paths import atomic_write_text, enforce_under_cwd_and_no_symlink
    from ai_forge_cli.utils.trust_remote import (
...
def evolve(
    ...
    from ai_forge_cli.utils.evolve import STRATEGIES, run_evolve
    from ai_forge_cli.utils.magpie import make_magpie_generate_fn
    from ai_forge_cli.utils.paths import atomic_write_text, enforce_under_cwd_and_no_symlink
...
def persona_mix(
    ...
    from ai_forge_cli.utils.paths import enforce_under_cwd_and_no_symlink
    from ai_forge_cli.utils.persona_hub import (
...
def brain_rot_cmd(
    ...
    from ai_forge_cli.utils.brain_rot import (
        refuse_if_rotten,
        score_dataset_brain_rot,
    )
    from ai_forge_cli.utils.brain_rot_lang import validate_lang_code
    from ai_forge_cli.utils.paths import enforce_under_cwd_and_no_symlink
