...
class DataConfig(BaseModel):
    ...
    def _validate_buffer_size_v042(cls, value: Optional[int]) -> Optional[int]:
        from ai_forge_cli.utils.data_pipeline import validate_buffer_size
    ...
    def _validate_shards_v042(cls, value: Optional[int]) -> Optional[int]:
        from ai_forge_cli.utils.data_pipeline import validate_shards
    ...
    def _validate_image_pixels_v042(cls, value, info):
        from ai_forge_cli.utils.data_pipeline import validate_image_pixels
    ...
    def _validate_video_fps_v042(cls, value: Optional[float]) -> Optional[float]:
        from ai_forge_cli.utils.data_pipeline import validate_video_fps
    ...
    def _validate_video_maxlen_v042(cls, value: Optional[int]) -> Optional[int]:
        from ai_forge_cli.utils.data_pipeline import validate_video_maxlen
    ...
    def _validate_new_tokens_v042(
        ...
        from ai_forge_cli.utils.data_pipeline import validate_new_tokens
    ...
    def _validate_prompt_strategy_v042(cls, value: Optional[str]) -> Optional[str]:
        from ai_forge_cli.utils.data_pipeline import validate_prompt_strategy
    ...
    def _validate_interleave_v042(cls, value):
        ...
        if isinstance(value, str):
            from ai_forge_cli.utils.data_pipeline import INTERLEAVE_STRATEGIES
...
class AdviseConfig(BaseModel):
    ...
    ``ai_forge_cli/commands/advise.py``; this field is informational on
    ``SoupConfig`` only.
    """
...
class TrainingConfig(BaseModel):
    ...
    optimizer: str = Field(
        ...
        description=(
            ...
            "See ai_forge_cli.utils.optimizer_zoo.SUPPORTED_OPTIMIZERS for the "
            "full list."
        ),
    )
    ...
    def _validate_minillm_anchor_path(cls, v):
        ...
        if v is None:
            return None
        from ai_forge_cli.utils.minillm import _check_path_shape
    ...
    def _normalize_hub(cls, v):
        ...
        from ai_forge_cli.utils.hubs import validate_hub_name
        if v is None:
            return v
    ...
    # resident. See ai_forge_cli.utils.layer_stream.
    ...
    def _validate_prequantized_no_qat(self) -> "TrainingConfig":
        ...
        """
        from ai_forge_cli.utils.quant_menu import is_quant_menu_format
    ...
    def _validate_optimizer(cls, value: str) -> str:
        ...
        from ai_forge_cli.utils.optimizer_zoo import validate_optimizer_name
    ...
    def _validate_lr_groups(cls, value):
        ...
        if value is None:
            return None
        from ai_forge_cli.utils.lr_groups import parse_lr_groups
    ...
    def _validate_expand_layers_field(cls, value):
        ...
        if value is None:
            return None
        from ai_forge_cli.utils.block_expansion import validate_expand_layers
    ...
    def _validate_unlearn_method(cls, v):
        ...
        if v is None:
            return None
        from ai_forge_cli.utils.unlearning import validate_unlearn_method
    ...
    def _validate_unlearn_alpha(cls, v):
        ...
        if v is None:
            return None
        from ai_forge_cli.utils.unlearning import validate_unlearn_alpha
    ...
    def _validate_citation_style(cls, v):
        ...
        if v is None:
            return None
        from ai_forge_cli.utils.citation_faithful import validate_citation_style
    ...
    def _validate_citation_recall_threshold(cls, v):
        ...
        if v is None:
            return None
        from ai_forge_cli.utils.citation_faithful import validate_citation_threshold
    ...
    def _validate_grace_codebook_size(cls, v):
        ...
        if v is None:
            return None
        from ai_forge_cli.utils.grace_codebook import validate_grace_codebook_size
    ...
    def _validate_grace_codebook_dim(cls, v):
        ...
        if v is None:
            return None
        from ai_forge_cli.utils.grace_codebook import validate_grace_codebook_dim
...
class ShipConfig(BaseModel):
    ...
    # Keep these values in sync with ai_forge_cli.utils.ship_verdict.TASK_MODES.
...
def _validate_reward_hack_controller(tcfg: Any) -> None:
    ...
    from ai_forge_cli.utils.reward_hack_control import SIGNAL_NAMES
...
class SoupConfig(BaseModel):
    ...
    def _validate_v028_speed_memory_supported_tasks(self) -> "SoupConfig":
        ...
        raise ValueError(
            ...
            "ai_forge_cli.utils.v028_features.supports_v028_features."
        )
    ...
    def _validate_multipack_supported_tasks(self) -> "SoupConfig":
        ...
        if not self.training.multipack:
            return self
        from ai_forge_cli.utils.multipack import supports_multipack
    ...
    def _validate_long_context_grpo(self) -> "SoupConfig":
        ...
        if not self.training.long_context_grpo:
            return self
        from ai_forge_cli.utils.grpo_long_context import (
    ...
    def _validate_prm_compat(self) -> "SoupConfig":
        ...
        if self.task != "prm":
            return self
        from ai_forge_cli.utils.prm import validate_prm_compat
    ...
    def _validate_vision_grpo(self) -> "SoupConfig":
        ...
        if not self.training.vision_grpo:
            return self
        from ai_forge_cli.utils.prm import validate_vision_grpo_compat
    ...
    def _validate_tts_compat(self) -> "SoupConfig":
        ...
        if self.task != "tts" and tcfg.tts_family is None and tcfg.tts_emotion is None:
            return self
        if self.task == "tts":
            from ai_forge_cli.utils.tts import (
    ...
    def _validate_classifier_compat(self) -> "SoupConfig":
        ...
        if self.task not in classifier_tasks and not classifier_fields_set:
            return self
        from ai_forge_cli.utils.classifier import (
            is_classifier_task,
    ...
    def _validate_unlearn_compat(self) -> "SoupConfig":
        ...
        from ai_forge_cli.utils.unlearning import validate_unlearn_compat
...
# The canonical source is `ai_forge_cli/templates/*.yaml` with `manifest.json`.
...
task: sft
# backend: unsloth  # 2-5x faster, pip install "ai-forge[fast]"
...
training:
  ...
  use_flash_attn: true
  # use_liger: true       # pip install "ai-forge[liger]" for fused ops
