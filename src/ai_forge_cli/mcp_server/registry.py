...
``(dict) -> dict``), and the shared security guards. :mod:`ai_forge_cli.mcp_server.server`
...
from ai_forge_cli.mcp_server.execution import (
    ExecutionError,
    ExecutionManager,
    ProtectedFile,
    digest_file,
)
from ai_forge_cli.utils.paths import enforce_under_cwd_and_no_symlink, is_under_cwd
...
def tool_advise(args: dict) -> dict:
    ...
    from ai_forge_cli.utils import advise as _advise
...
def tool_data_doctor(args: dict) -> dict:
    ...
    from ai_forge_cli.data import formats as _formats
    from ai_forge_cli.utils import data_doctor as _dd
    ...
    try:
        ...
        raise McpToolError(
            "data_doctor needs the tokenizer stack: pip install \"ai-forge[train]\""
        ) from exc
    try:
...
def tool_recipes_search(args: dict) -> dict:
    ...
    from ai_forge_cli.recipes.catalog import RECIPES, search_recipes
...
def tool_profile(args: dict) -> dict:
    ...
    from ai_forge_cli.utils.profiler import (
        estimate_speed,
        estimate_total,
...
def tool_diagnose_evidence(args: dict) -> dict:
    ...
    from ai_forge_cli.utils.diagnose.report import FAILURE_MODES, FailureScore, classify_score
    ...
    return [
        ...
        ToolSpec(
            ...
            description=(
                ...
                "BOS dup, truncation risk). Needs the ai-forge[train] tokenizer stack."
            ),
            input_schema={
                "type": "object",
                "properties": {
